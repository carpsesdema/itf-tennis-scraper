"""Flashscore scraper implementation - OPTIMIZED FOR SLOW COMPUTERS."""
...
class FlashscoreScraper(BaseScraper):
    ...
    async def _process_match_fast(self, sel_element: ElementHandle, element_index: int,
        ...
        """FAST match processing for slow computers."""
        try:
            ...
            match_id = f"flashscore_bet365_{element_index}_{hash(home_player_name + away_player_name) % 10000}"
    ...
    async def scrape_matches(self) -> ScrapingResult:
        ...
        self.logger.info(f"🎯 OPTIMIZED SCRAPING for slow computer - Max {self.MAX_MATCHES_TO_PROCESS} matches")
        ...
        try:
            ...
            self.logger.info(f"🔍 Looking for match elements (max {self.MAX_ELEMENTS_TO_CHECK})...")
            ...
            if not match_elements:
                ...
                # Process elements with STRICT LIMIT
                for element_index, sel_element in enumerate(match_elements):
                    # STOP if we hit our match limit
                    if bet365_matches_found >= self.MAX_MATCHES_TO_PROCESS:
                        self.logger.info(f"🛑 Reached match limit ({self.MAX_MATCHES_TO_PROCESS}) - stopping")
                        break
                    ...
                    try:
                        ...
                        if match_obj:
                            ...
                            if match_obj.metadata.get('is_match_tie_break'):
                                self.logger.critical(
                                    f"🚨🚨🚨 TIE BREAK #{bet365_matches_found}: {match_obj.home_player.name} vs {match_obj.away_player.name}")
                            else:
                                self.logger.info(
                ...
                success = True
                self.logger.info(f"✅ OPTIMIZED SCRAPING COMPLETE!")
                self.logger.info(f"📊 Processed: {processed_elements_count} elements")
                self.logger.info(f"🎯 Found: {len(matches_found)} bet365 matches")

                tie_break_count = len([m for m in matches_found if m.metadata.get('is_match_tie_break')])
                if tie_break_count > 0:
                    self.logger.critical(f"🚨 {tie_break_count} TIE BREAK MATCHES FOUND!")
